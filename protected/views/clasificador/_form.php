<?php
/* @var $this ClasificadorController */
/* @var $model Clasificador */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'clasificador-form',
	'enableAjaxValidation'=>false,
)); ?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	<div class="row">
		<?php echo $form->labelEx($model,'CODIGOCLASIFICADOR'); ?>
		<?php echo $form->textField($model,'CODIGOCLASIFICADOR',array('size'=>20,'maxlength'=>20)); ?>
		<?php echo $form->error($model,'CODIGOCLASIFICADOR'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'CLA_descripcion'); ?>
		<?php echo $form->textField($model,'CLA_descripcion',array('size'=>60,'maxlength'=>150)); ?>
		<?php echo $form->error($model,'CLA_descripcion'); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->